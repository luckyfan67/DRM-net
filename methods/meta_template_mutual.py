...
class MetaTemplate(nn.Module):
  ...
  def correct(self, x):
    scores, loss, mu_loss = self.set_forward_loss(x)
  ...
  def train_loop(self, epoch, train_loader, optimizer, total_it):
    ...
    for i, (x,_ ) in enumerate(train_loader):
      ...
      if self.change_way:
        self.n_way  = x.size(0)
      optimizer.zero_grad()
      _, loss, mu_loss = self.set_forward_loss(x)
      loss.backward(retain_graph=True)
      mu_loss.backward()
      optimizer.step()
      avg_loss = avg_loss+loss.item()
      avg_muloss = avg_muloss+mu_loss.item()

      if (i + 1) % print_freq==0:
        print('Epoch {:d} | Batch {:d}/{:d} | Loss {:f} | mu_Loss {:f}'.format(epoch, i + 1, len(train_loader),avg_loss/float(i+1),avg_muloss/float(i+1)))
  ...
  def test_loop(self, test_loader, record = None):
    ...
    with open('/public/qifan_Data/few-shot/CrossDomainFewShot-master/output/results/tnnls_ori_1s_acc.txt','a') as f_acc:
        f_acc.write(' \n ' + str(acc_mean))
                      
    with open('/public/qifan_Data/few-shot/CrossDomainFewShot-master/output/results/tnnls_ori_1s_loss.txt','a') as f_loss:
        f_loss.write(' \n ' + str(loss/count))
